...
describe("@cli-blog/cli run", () => {
  ...
  test("maps editorial flags into post create bodies", async () => {
    ...
    await run({
      ...
      env: {
        CLI_BLOG_API_KEY: "cli_blog_sk_test",
  ...
  test("supports revisions, redirects, sitemap, and author avatar media commands", async () => {
    ...
    await run({
      ...
      env: { CLI_BLOG_API_KEY: "cli_blog_sk_test", CLI_BLOG_API_URL: "https://api.example.com" },
    });
    await run({
    ...
    const sitemap = await run({
      ...
      env: { CLI_BLOG_API_KEY: "cli_blog_pk_test", CLI_BLOG_API_URL: "https://api.example.com" },
    });
